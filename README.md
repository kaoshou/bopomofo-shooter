...
*   **多元輸入模式支援 (Multiple Input Modes)**：
    *   **滑鼠模式 (Mouse Mode)**：使用滑鼠移動，左鍵射擊，具備最高的系統相容性。
    *   **鏡頭體感模式 (Webcam Motion Mode)**：免購買硬體！只需舉起單手手掌即可控制畫面準星，在空中做出「握拳」動作即可觸發射擊。
        *   **軌跡鎖定防干擾 (Track ID Locking)**：內建多目標軌跡追蹤演算法，精準鎖定控制手。一旦鎖定，旁人伸手或自己的另一隻手揮舞皆無法搶走控制權。
        *   **無縫換手控制 (Seamless Hand Swap)**：支援隨時換手！只要將原本的手放下，系統會在 0.3 秒後自動解鎖，允許改舉起另一隻手（左/右手）繼續遊玩。
        *   **雙人實體分區與雙倍映射 (2P Split & Double Mapping)**：雙人模式下將 Webcam 實體畫面左右切割（預覽畫面底部有高對比黃色提示線），兩側動作放大雙倍映射到全螢幕，徹底防範雙手錯配，且兩名玩家皆能自由瞄準全螢幕。
    *   **光線槍模式 (Lightgun Mode)**：免去繁瑣的校正步驟，隨插即用（支援 GUN4IR、OpenFIRE 等相容 Gamepad API 的光線槍設備，建議切換全螢幕）。
*   **雙人遊玩支援 (2P Play)**：
    *   **2P 合作模式**（支援光線槍與鏡頭體感）：兩名玩家同畫面擊碎氣球，共享分數與 Combo 連擊加成，共同爭奪最高評分與力拼默契評價。
    *   **2P 對抗模式**（支援光線槍與鏡頭體感）：兩名玩家同畫面競技搶答，擊中正確氣球即可得分並切換下一題（防範對方搶答），擊中錯誤氣球會扣分，限時結束看誰勝出。
*   **體感自我修復防卡死系統 (Motion Anti-Freeze & Self-Healing)**：系統具備雙重自動修復功能。若 MediaPipe 偵測引擎超時（1000ms）或出錯，系統會在 3 秒內自動重建偵測實例；若 Webcam 視訊串流卡死（連續 2 秒播放時間無變化），系統將自動重啟鏡頭串流。全程於背景無縫完成，無須重新整理網頁。
*   **射擊防抖冷卻系統 (Debounce/Cooldown)**：核心內建 250 毫秒射擊冷卻限制，能自動過濾視訊鏡頭體感偵測雜訊與實體光槍扳機的物理彈跳，避免「無效空槍」稀釋命中率。
*   **個人化配置設定 (Custom Settings)**：支援語音重播間隔調整、題目語音/背景音樂/遊戲音效獨立音量控制、氣球下降速度（提供慢速/標準/快速/超快/極速/瘋狂六種選擇）以及光線槍抗抖平滑度調整。
